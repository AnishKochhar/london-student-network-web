...
import { disconnectInstagramDB, deleteInstagramDataDB } from "@/app/lib/data";
...
export async function POST(request: Request) {
  try {
    ...
    const result3 = await deleteInstagramDataDB(LSNUserId);

    if (!result3.success) {
        return NextResponse.json({ error: `Failed to delete user Instagram data from database for user: ${userIdToDelete}` }, { status: 500 });
    }
