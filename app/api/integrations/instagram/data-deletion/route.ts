...
export async function POST(req: Request) {
  try {
    ...
    const result3 = await deleteInstagramDataDB(userId);

    if (!result3.success) {
        return NextResponse.json({ error: "Failed to delete user Instagram data from database, for user:", userId }, { status: 500 });
    }
