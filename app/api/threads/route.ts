...
export async function GET(request: NextRequest) {
    try {
        ...
        const myThreadsFilter = searchParams.get("myThreads") === "true";
        ...
        `;
                ...
                searchConditions.push(
                    `(t.title ILIKE $${paramIndex} OR t.content ILIKE $${paramIndex})`,
                );
        ...
        } else if (tagFilters.length > 0 || authorFilter || myThreadsFilter) {
            // Add joins for filters even without search term
            fromClause = `
        FROM threads t
        LEFT JOIN thread_tags tt ON t.id = tt.thread_id
        LEFT JOIN forum_tags ft ON tt.forum_tag_id = ft.id
      ...
      `;
        ...
        // Handle "My Threads" filter
        if (myThreadsFilter && userId) {
            whereConditions.push(`t.author_id = $${paramIndex}`);
        ...
        switch (sortBy) {
            case "Most Popular":
                additionalSelect =
                    ", (t.upvotes - t.downvotes) as popularity_score";
                orderByClause =
                    "ORDER BY popularity_score DESC, created_at DESC";
                break;
            case "Most Replies":
                additionalSelect =
                    ", (SELECT COUNT(*) FROM comments c WHERE c.thread_id = t.id AND c.parent_id IS NULL) as reply_count";
    ...
    `;
        ...
        const threadsResult = await sql.query(threadsQuery, [
            ...parameters,
            limit,
            offset,
        ]);
    ...
    `;
        ...
        const countResult = await sql.query(countQuery, parameters);
