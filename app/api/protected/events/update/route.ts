...
export async function POST(req: Request) {
    try {
        const { eventId, formData }: { eventId: string; formData: FormData } =
            await req.json();
        ...
        if (userId) {
            ...
            if (accessGranted) {
                // old event route + frontend image upload logic goes here, to be robust against malicious users
                ...
                if (
                    ...
                    typeof formData?.uploadedImage !== "string"
                ) {
                    try {
        ...
        console.error(
            "Error verifying requester, there was an error in session extraction, or ownership verification",
            error,
        );
        return NextResponse.json(
