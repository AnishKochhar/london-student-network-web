...
export async function fetchThreadReplies(
    ...
    page: number = 1,
    limit: number = 10,
): Promise<{ replies: Reply[]; topLevelCount: number }> {
    try {
        const url = new URL(
...
): Promise<Partial<ThreadData> | null> {
    try {
...
// Submit votes
export async function submitThreadVote(
    threadId: number,
...
export async function submitReplyVote(
    replyId: number,
    voteType: "upvote" | "downvote",
    isRemovingVote: boolean,
): Promise<boolean> {
