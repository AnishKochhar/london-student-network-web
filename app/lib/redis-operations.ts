...
export async function insertToken(
    ...
    try {
        ...
        if (
            ...
            console.log(
                `Verification token for email ${email} inserted/updated successfully.`,
            );
            return { success: true };
        } else {
            // Handle cases where one or both SET operations failed
            console.error(
                "Failed to set both token and email in Redis:",
                results,
            );
            return { success: false };
        }
...
export async function getEmailFromToken(token: string, type: string) {
    // type is 'verification' or 'reset_password'
    try {
