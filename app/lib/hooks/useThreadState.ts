...
type ThreadStateAction =
    ...
    | {
          type: "UPDATE_THREAD";
          payload: { threadId: number; data: Partial<ThreadData> };
      }
    | {
          type: "UPDATE_REPLY";
          payload: { replyId: number; data: CommentUpdateData };
      }
    ...
    switch (action.type) {
        ...
        case "SET_REPLIES":
            return {
                ...state,
                thread: state.thread
                    ? { ...state.thread, replies: action.payload }
                    : null,
            };
...
export function useThreadState(initialThread: ThreadData | null) {
    ...
    const updateThread = useCallback(
        (threadId: number, data: Partial<ThreadData>) => {
    ...
    const updateReply = useCallback(
        (replyId: number, data: CommentUpdateData) => {
