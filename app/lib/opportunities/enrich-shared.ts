/**
 * Shared enrichment contract — the extraction prompt, JSON schema, and
 * response mapping used by every LLM provider (Claude, Groq, …). Keeping these
 * in one place means a prompt tweak can't silently drift between providers, and
 * a new provider only has to supply the transport (how it calls its API); the
 * prompt, schema, and parsing are identical across all of them.
 */
...
export const LOCATION_TYPES = ["on_site", "hybrid", "remote"];

export const MAX_INPUT_CHARS = 12_000; // bound tokens/cost per posting

// JSON schema the model's output is constrained to. Strict-compatible for both
// Anthropic (output_config.format) and OpenAI-style (response_format.json_schema):
// additionalProperties:false and every field listed in `required`.
export const SCHEMA: Record<string, unknown> = {
    ...
    properties: {
        ...
        deadline: {
            type: ["string", "null"],
            description:
                "Application deadline as an ISO 8601 date, or null if not stated",
        },
...
export const SYSTEM = `You are an extraction engine for LSN, a London student opportunities platform.
...
- A full-time graduate, junior, or entry-level role at the actual employer (i.e. not a staffing agency) IS a genuine student opportunity and should score relevance normally — do NOT down-rank it merely for being full-time, being titled "Junior", or requiring a degree. Titles like "Graduate <role>", "Junior <role>", "<role> Intern", or "<role>, Analyst/Developer/Researcher" at the hiring company are exactly what final-year students and recent graduates apply to.
...
/** Build the user-turn text from a raw scraped posting. */
export function buildUserText(input: EnrichmentInput): string {
    return [
...
/**
 * Map a model's raw JSON-schema output into an EnrichmentResult. Identical for
 * every provider — the schema guarantees the shape, this coerces/clamps it.
 * `rawJson` is the model's response text; `model` labels the reasoning trail.
 */
