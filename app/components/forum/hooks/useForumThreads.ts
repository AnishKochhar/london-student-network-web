...
export function useForumThreads(searchTerm: string, sortBy: string) {
  ...
  const fetchThreads = useCallback(async () => {
    ...
    try {
      ...
      const transformedThreads = (data.threads || []).map(thread => ({
        ...
        replyCount: thread.replyCount !== undefined ? thread.replyCount : 0
      }));
  ...
  const fetchMoreThreads = useCallback(async () => {
    ...
    try {
      ...
      if (searchTerm === searchTermRef.current && sortBy === sortByRef.current) {
                const transformedThreads = (data.threads || []).map(thread => ({
          ...thread,
          replyCount: thread.replyCount !== undefined ? thread.replyCount : 0
        }));
  ...
  }, [page, searchTerm, sortBy, hasMorePosts, isLoadingMore]);
  ...
  // Handle search/sort changes with a separate effect
  useEffect(() => {
    // Only refetch if search term or sort order has actually changed
    if (searchTerm !== searchTermRef.current || sortBy !== sortByRef.current) {
      fetchThreads();
    }
  }, [searchTerm, sortBy, fetchThreads]);
