...
export default function ModernSocietyRegistration() {
	...
	const onSubmit = async (data: SocietyRegisterFormData) => {
		...
		try {
			...
			if (nameResult.nameTaken) {
				...
				setErrorModal({
					...
					message: 'A society with this name already exists. Please choose a different name.'
				});
				return;
			}
		} catch (error) {
			...
			setErrorModal({
				isOpen: true,
				title: 'Connection Error',
				message: 'Unable to verify society name. Please check your connection and try again.'
			});
			return;
		}
		...
		try {
			...
			if (result.success) {
				toast.success('Society account created successfully!', { id: toastId });
					...
					<div className="space-y-4">
						...
						{showOtherUniversity && (
							<ModernInput
								...
								{...register('otherUniversity', { required: 'Please specify your institution' })}
							/>
						)}
					</div>
				</ModernFormStep>
					...
					<div className="space-y-6">
						<div className="space-y-4">
							<div className="space-y-2">
								<label className="text-gray-300 text-left block">Society Description (optional)</label>
								<textarea
						...
						<div className="space-y-4">
							<label className="text-gray-300 text-left block">Tags (select up to 3)</label>
							<Controller
								name="tags"
								control={control}
								render={({ field }) => (
