...
export default function ModernCompanyRegistration() {
    ...
    const canContinue = () => {
        switch (currentStep) {
            case 1:
                return (
                    watchedValues.companyName && watchedValues.hasAgreedToTerms
                );
            case 2:
                return (
    ...
    const handleAddMotivation = () => {
        if (
            newMotivation.trim() &&
            !motivationOptions.includes(newMotivation)
        ) {
    ...
    const onSubmit = async (data: CompanyRegisterFormData) => {
        const toastId = toast.loading(
            "Creating your organisation's account...",
        );
        ...
        try {
            ...
            setErrorModal({
                ...
                title: "Connection Error",
                message:
                    "Unable to verify company name. Please check your connection and try again.",
            });
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                toast.success("Organisation account created successfully!", {
                    id: toastId,
                });
            ...
            {currentStep === 5 && (
                ...
                >
                    <div className="space-y-6">
                        <div className="space-y-4">
                            <label className="text-gray-300 text-left block">
                                What will you be using LSN for?
                            </label>
                            <div className="grid grid-cols-1 gap-3">
                                {motivationOptions.map((option) => (
                                    <label
                                        key={option}
                                        className="flex items-center space-x-3 p-3 sm:p-4 bg-white/10 rounded-xl border border-white/20 hover:bg-white/20 transition-all cursor-pointer"
                                    >
                                        <div className="relative flex-shrink-0">
                                            <input
                                                type="checkbox"
                                                className="sr-only"
                                                checked={motivation.includes(
                                                    option,
                                                )}
                                                onChange={(e) =>
                            ...
                            <div className="mt-4 p-3 sm:p-4 bg-white/5 rounded-xl border border-white/10">
                                <label className="text-gray-300 text-left block text-sm mb-3">
                                    Add custom motivation (optional)
                                </label>
                                ...
                                {newMotivation.trim() && (
                                    <p className="text-gray-400 text-xs mt-2">
                                        This will be added to your motivations
                                        list
                                    </p>
                                )}
                            </div>
