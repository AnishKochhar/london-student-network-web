...
export default function EditEventComponent({
    ...
    const [viewSendEmailsModal, setViewSendEmailsModal] =
        useState<boolean>(false); // this should be in conflict with viewRegistrations, only one can be true at the same time
    ...
    const mapEventToFormData = (event: Event): FormData => {
        const [startHour, startMinute] = event.time
            .split(" - ")[0]
            .split(":")
            .map(Number);
        const [endHour, endMinute] = event.time
            .split(" - ")[1]
            .split(":")
            .map(Number);
    ...
    useEffect(() => {
        ...
        Object.keys(formData).forEach((key) => {
            ...
            if (
                typeof formData[formDataKey] === "object" &&
                formData[formDataKey] !== null
            ) {
                // Nested objects
                Object.keys(formData[formDataKey] as object).forEach(
                    (nestedKey) => {
                        setValue(
                            `${key}.${nestedKey}` as keyof FormData,
                            formData[formDataKey][nestedKey],
                        );
                    },
                );
            } else {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            console.log(data);
            const response = await fetch("/api/protected/events/update", {
                // upload of image is handled in here
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            toast.error(`Error occured trying to update event: ${error}.`, {
                id: toastId,
            });
        }
    };
    ...
    const TitleField = () => (
        <div className="flex flex-col mb-4">
            <label htmlFor="title" className="text-2xl p-6 font-semibold">
                Title
            </label>
            {errors.title && (
    ...
    const OrganiserField = () => (
        <div className="flex flex-col mb-4">
            <label htmlFor="organiser" className="text-2xl p-6 font-semibold">
                Organiser
            </label>
            <select
                id="organiser"
            ...
            {errors.organiser && (
                <p className="text-red-600">Organiser is required</p>
            )}
        </div>
    );
    ...
    const DateField = () => {
        ...
        return (
            <div className="flex flex-col space-x-2 mb-4">
                <label htmlFor="date" className="text-2xl p-6 font-semibold">
                    Date
                </label>
                <div className="flex flex-row justify-end self-end w-auto p-2 space-x-2 ">
                    <div className="flex flex-col p-2">
                        ...
                        <select
                            {...register("date.day", { required: true })}
                    ...
                    <div className="flex flex-col p-2">
                        ...
                        <select
                            {...register("date.month", { required: true })}
                            className="w-full bg-transparent"
                    ...
                    <div className="flex flex-col p-2">
                        ...
                        <select
                            {...register("date.year", { required: true })}
                            className="w-full bg-transparent"
    ...
    const TimeField = () => {
        ...
        return (
            <div className="flex flex-col mb-4">
                <label htmlFor="time" className="text-2xl p-6 font-semibold">
                    Time
                </label>
                <div className="flex flex-col sm:flex-row justify-end w-auto self-end p-2 space-x-2">
                    <div className="flex flex-col items-center p-2">
                        <label className="text-sm font-semibold mb-1 text-center w-full">
                            Start Time
                        </label>
                        <div className="flex space-x-2">
                            <select
                                {...register("time.startHour", {
                                    required: true,
                                })}
    ...
    const TagsFieldWrapper = () => {
        ...
        return (
            <TagsField
                value={eventTagValue}
                onChange={(newTagValue: number) =>
                    setValue("event_tag", newTagValue)
                }
            />
        );
    };
    ...
    const ImagePickerField = () => {
        ...
        const selectedImage = watch(
            "selectedImage",
            "/images/placeholders/lecture-hall-1.jpg",
        );
        ...
        return (
            <div className="flex flex-col mb-4">
                ...
                <div className="flex flex-col w-[50%] self-end">
                    <p className="text-sm self-end text-gray-500 mb-2">
                        Please choose from one of our placeholders
                    </p>
                    <select
                    ...
                    <div className="self-end flex flex-col w-full">
                        <button
                            className="my-2 self-end w-fit px-4 items-center font-light text-gray-700 border border-gray-300 hover:bg-gray-200 rounded-sm text-sm h-10"
    ...
    const LocationField = () => (
        <div className="flex flex-col mb-4">
            ...
            <div className="flex flex-col mb-4 w-full">
                <label
                    htmlFor="building"
                    className="text-md font-semibold mb-2 ml-10"
                >
                    Building
                </label>
                <Input
    ...
    const CapacityField = () => (
        <div className="flex flex-col mb-4">
            <label htmlFor="capacity" className="text-2xl p-6 font-semibold">
                Ticketing Capacity
            </label>
            {errors.capacity && (
                <p className="text-red-600 text-sm self-end mb-1">
    ...
    const SignupLinkField = () => (
        <div className="flex flex-col mb-4">
            <label className="flex flex-row items-center">
                <p className="text-2xl p-6 font-semibold">Sign-up link</p>{" "}
                <p className="text-lg p-2">(optional)</p>
            </label>
            <Input
                type="url"
                {...register("signupLink")}
        ...
        <div className="flex flex-col mb-4">
            <label className="flex flex-row items-center">
                <p className="text-2xl p-6 font-semibold">
                    Please provide any information for external students
                </p>{" "}
                <p className="text-lg p-2">(optional)</p>
            </label>
            {errors.description && (
