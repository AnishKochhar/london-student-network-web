...
const TextRevealSection: FC<TextRevealSectionProps> = ({
    ...
    return (
        <div
            ref={targetRef}
            className={cn(
                "relative z-0 h-[50vh] flex items-center justify-center w-full max-w-[70%] md:max-w-[70%] sm:max-w-full",
                className,
            )}
        >
            <div
            ...
            >
                {texts.map((text, i) => (
                    <Fragment key={i}>
                        {text.split(" ").map((word, j) => {
                            ...
                            return (
                                <Word
                                    ...
                                    range={[start, end]}
                                    unrevealedTextColor={unrevealedTextColor}
                                    revealedTextColor={revealedTextColor}
                                >
                                    {word}
                                </Word>
                            );
                        ...
                        {i < texts.length - 1 && (
                            <div className="basis-full h-8"></div>
                        )}
                    </Fragment>
                ))}
            </div>
...
const Word: FC<WordProps> = ({
    ...
    return (
        <span className="xl:lg-3 relative mx-1 lg:mx-2.5">
            <span className={cn("absolute", unrevealedTextColor)}>
                {children}
            </span>
            <motion.span
